  ...
  <el-dialog :visible.sync="dialogVisible" width="470px" v-loading="showLoading" @open="handleDialogOpen()">
    ...
    <div class="user-list-container">
      <div class="user-container" v-for="(item, index) in this.pagingUserList.rows" :key="item.id" :class="{'selected-user' : selectedIndex === index}" @click="selectUser(item, index)">
        <div class="avatar" v-if="item.avatarUrl"> <img width="40px" height="40px" :src="item | avatar" ></div>
        <div class="avatar" v-if="!item.avatarUrl">{{ item.firstLetterOfName.toUpperCase() }}</div>
...
export default {
  ...
  filters:{
    avatar(item){
      if(item.avatarUrl){
        return window.BASE_API + '/users/' + item.id + '/avatar?width=80&height=80&rdm='
      }else {
        return "";
      }
    }
