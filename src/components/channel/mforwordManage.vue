<template>
  <el-dialog :visible.sync="dialogVisible" :modal-append-to-body="false" width="100%" v-loading="loadingVisible">
    ...
    <div class="list-container">
      <div class="user-list-container left-list">
        ...
        <ul v-if="type == 0">
          <li :class="{'selected':item.forword}" v-for="(item, index) in this.userChannelList"  @click="selectUser(item, index)">
             <i v-if="item.channelType == 'G'">(群)</i>
            <i v-if="item.channelType == 'P'">(私)</i>
            {{ item.channelAliasName || item.channelName }}</li>
        </ul>
        <ul v-if="type == 1">
          <li :class="{'selected':item.forword}" v-for="(item, index) in this.userChannelList" v-if="item.channelType == 'G'"  @click="selectUser(item, index)">
            <i v-if="item.channelType == 'G'">(群)</i>
            <i v-if="item.channelType == 'P'">(私)</i>
            {{ item.channelAliasName || item.channelName }}</li>
        </ul>
        <ul v-if="type == 2">
          <li :class="{'selected':item.forword}" v-for="(item, index) in this.userChannelList" v-if="item.channelType == 'P'"  @click="selectUser(item, index)">
             <i v-if="item.channelType == 'G'">(群)</i>
            <i v-if="item.channelType == 'P'">(私)</i>
            {{ item.channelAliasName || item.channelName }}</li>
        </ul>
...
export default {
  ...
  mounted: function() {
    this.$root.Event.$on('mopenforwordDialog',(userChannel,selectChat)=>{
      for(var i = 0; i < this.userChannelList.length;i++){
        this.userChannelList[i].forword = false;
      }
