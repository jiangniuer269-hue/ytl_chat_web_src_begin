<template>
  <div class="container">
    ...
    <div class="footer" style="padding-bottom: 5px;">
      <msend-message ref="messagesend" :user-channel="userChannel" :channel-id="$route.params.channelId" :channel-type="$route.params.channelType" @onMessageSent="showSentMessage"/>
    </div>
  </div>
</template>
  ...
  created() {
    var that = this;
     document.onkeydown = function(e) {   //按下回车提交
        ...
        //事件中keycode=13为回车事件
        if (key == 13 && document.activeElement.id == "inpt-area") {
            that.$refs.messagesend.doSendMessage();
        }
    };
