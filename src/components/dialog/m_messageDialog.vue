<template>
  <div class="container">
    <div class="header">
      <i class="el-icon-arrow-left goback" @click="goback"><span class="unread" v-show="unreadCount != 0">({{unreadCount}})</span></i>
      <div v-if="!userChannel.toUserSignature" style="float:left;">
        <span class="channelName"> {{userChannel.channelAliasName || userChannel.toUserNickname || userChannel.channelDisplayName}}</span>
      </div>
      ...
      <template v-if="userChannel.channelType === 'P'">
        <el-button style="float:right;margin-right:10px;margin-top:10px;" v-if="(origin == 'sys' || origin == 'pc') && selectChat.length" type="danger" size="mini" @click="doForward()">转发</el-button>
...
export default {
  ...
  methods: {
    doForward(){
      this.$root.Event.$emit("mopenforwordDialog", this.userChannel,this.selectChat);
    },
    isManage(id){
    ...
    initPage() {
      ...
      this.$nextTick(function(){
        if(this.$router.history.current.name == 'm'){
              ...
              }else if(this.$router.history.current.name == 'm_messageDialog'){
                $(".sidebar").hide()
              }
      })
  ...
  created() {
    // if(!this.$store.getters.imClient){
